...
% Dopplerzaman dilimlerinin sayısı:
M = 4;

%  Gecikme dilimlerinin alt taşıyıcıları sayısı:
N = 2;

% Modülasyon boyutu:
mod_size = 4;

% FFT(Fast Fourier Transform -Hızlı Fourier Dönüşümü) Matrisi:
Fn = dftmtx(N);
Fm = dftmtx(M);
...
% N x M OTFS Gecikme-Doppler çerçevesini oluşturma:
...
%% Gecikme - Doppler Analizi:
% Gecikme - Doppler Kanal Matrisi H kullanılarak Gecikme - Doppler Veri Yanıtı :
...
pmf = (1 / mod_size) * ones(N * M, N * M, mod_size); % Bu P(c,d) olacak şekilde olmalıdır.
pmf1 = ones(N * M, N * M, mod_size); % Bu P(c,d) olacaktır.
pmf2 = ones(N * M, mod_size); % Bu P(c) olacaktır.
...
pmf1 = ones(N * M, N * M, mod_size); % Bu P(c,d) olmalıdır.
pmf2 = ones(N * M, mod_size); % Bu P(c) olmalıdır.

end % İterasyonların sonu.
